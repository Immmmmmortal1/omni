...
const COPY: Record<Lang, Copy> = {
  cn: {
    ...
    diagramHint: "端侧视觉 + 端侧 Whisper · 云端人设 · 流式语音",
    diagram: DIAGRAM_CN,
    openKicker: "开源",
    openTitle: "我们是开源的 ✿",
    openCopy:
    ...
    footerBegin: "开始",
    visionKicker: "完整愿景",
    visionWarn: "⚠ 黑客松 · 没做完的部分",
    visionTitle: "给万物一个灵魂 ✿",
    visionLede: "demo 是情绪搭子。完整产品是一个基础模型。",
    visionBody:
      "核心理念很简单：每个物品都有值得学习的东西。我们想做的是一个基础模型，把物理世界变成随时可查的知识源。你选滤镜 — 切到「语言」，你就可以直接和周围的东西用目标语言对话：指着杯子问它怎么说、让它用西语聊它自己、跟它来回练口语，教室就是你的房间。切到「历史」，街边的长椅讲这条街的变迁。切到「陪伴」，就是你现在玩的这个。用户决定要从万物里提取什么。",
    visionClosing:
      "这次时间只够把基础做到真正跑得动、够稳、够可靠。剩下的，再一天就能上。",
    visionNextBadge: "下一步",
    visionNext:
      "基础模型已经开源，任何人可以在上面继续搭。我们自己的下一步大概率是教育方向的 app — 难的基建已经在了，剩下只是针对具体场景微调。",
  },
  en: {
    ...
    visionLede:
      "the demo is the entertainment lens. the real product is a foundation model.",
    visionBody:
      "the idea is simple: every object around you has something worth learning from. we want to build a foundation model that turns the physical world into a knowledge source you can query on demand. you pick the lens. switch to language — and you can actually talk to the things around you in the language you\u2019re learning: point at a mug and ask it how to say its name, let it introduce itself in spanish, have a back-and-forth with it until the word sticks. your room becomes the classroom. switch to history — the bench on the corner tells you how this street changed. switch to play — that\u2019s what you\u2019re using now. the user decides what to extract.",
    visionClosing:
  ...
  return (
    <main className="relative min-h-[100svh] overflow-hidden">
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-[920px] flex-col">
        {/* Top rail */}
        <header className="flex items-center justify-between gap-2 px-4 pt-4 sm:px-6 sm:pt-5">
          <div className="flex items-baseline gap-1.5 sm:gap-2">
            <span className="h-2 w-2 shrink-0 rounded-full bg-[color:var(--accent)] shadow-[0_0_0_4px_rgba(236,72,153,0.18)]" />
            <span className="serif-italic text-[22px] font-semibold leading-none text-[color:var(--ink)] sm:text-[30px]">
              omni
            </span>
        ...
        <section className="px-4 pb-5 pt-5 sm:px-6 sm:pb-6 sm:pt-6">
          <div className="grid items-center gap-6 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] md:gap-8">
            <div className="text-center md:text-left">
              <h1 className="serif-italic text-balance text-[34px] font-semibold leading-[1.02] tracking-[-0.02em] text-[color:var(--ink)] sm:text-[54px] sm:leading-[0.98] md:text-[64px]">
                {isCN ? (
                  <>
                    {t.heroLine1}
                    <br />
                    <span className="relative inline-block">
                      {t.heroSoul}
                      <span className="absolute -right-4 -top-1 inline-block animate-[wiggle_2.2s_ease-in-out_infinite] text-[20px] text-[color:var(--accent)] sm:-right-10 sm:-top-2 sm:text-[44px]">
                        ✿
                      </span>
                    </span>
                  </>
                  ...
                  <>
                    ...
                    <span className="relative inline-block">
                      {t.heroSoul}
                      <span className="absolute -right-4 -top-1 inline-block animate-[wiggle_2.2s_ease-in-out_infinite] text-[20px] text-[color:var(--accent)] sm:-right-9 sm:-top-2 sm:text-[44px]">
                        ✿
                      </span>
                    </span>
                  </>
              ...
              <p className="mx-auto mt-3.5 max-w-[500px] text-balance text-[13.5px] leading-[1.55] text-[color:var(--ink-soft)] sm:mt-3 sm:text-[15px] sm:leading-[1.5] md:mx-0">
                {sub.prefix}
              ...
              <div className="relative mt-5 inline-flex w-full sm:mt-6 sm:w-auto">
                <div
                  aria-hidden
                  className="absolute -inset-4 rounded-[48px] opacity-70 blur-2xl sm:-inset-8"
                  style={{
                    background:
                ...
                <div className="relative flex w-full flex-col items-stretch gap-2 sm:w-auto sm:flex-row sm:items-center">
                  <Link
                    href="/?onboarding=1"
                    className="btn-primary px-5 py-3 text-center text-[13.5px] font-semibold tracking-wide sm:py-2.5 sm:text-[13px]"
                  >
                    {t.ctaBegin}
                  </Link>
                  <Link
                    href="/?lens=play"
                    className="btn-ghost px-4 py-3 text-center text-[12.5px] font-semibold tracking-wide sm:py-2.5"
                  >
                    {t.ctaSkip}
                  </Link>
                </div>
              ...
              <div className="mt-4 hidden items-center justify-center gap-3 text-[color:var(--ink-muted)] sm:flex md:justify-start">
            ...
            <div className="relative mx-auto w-full max-w-[360px] sm:max-w-[320px] md:max-w-[380px]">
              <div
                aria-hidden
        ...
        {/* Open source — big CTA */}
        <section className="order-[90] px-4 pb-5 sm:order-none sm:px-6">
          <a
            href={GITHUB_URL}
            target="_blank"
        ...
        <section className="px-4 pb-5 sm:px-6">
          ...
          <div className="overflow-x-auto rounded-[18px] bg-white/80 p-3 shadow-[0_14px_28px_-22px_rgba(42,21,64,0.28)] ring-1 ring-white/80 backdrop-blur-md sm:rounded-[20px] sm:p-5">
            <MermaidDiagram
              chart={t.diagram}
              className="mx-auto flex min-w-[900px] justify-center [&_svg]:h-auto [&_svg]:max-w-full"
            />
          </div>
          <p className="mt-1.5 text-center text-[9.5px] font-medium uppercase tracking-[0.22em] text-[color:var(--ink-muted)] sm:hidden">
            ← {isCN ? "滑动查看" : "swipe to explore"} →
          </p>
        </section>
        ...
        <section className="px-4 pb-5 sm:px-6">
          ...
          <div className="-mx-4 flex snap-x snap-mandatory gap-2.5 overflow-x-auto px-4 pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:mx-0 sm:grid sm:grid-cols-3 sm:gap-3 sm:overflow-visible sm:px-0">
            {["clip1", "clip2", "clip3"].map((name, i) => (
              <div
                key={name}
                className="group relative w-[62%] shrink-0 snap-center overflow-hidden rounded-[16px] bg-black/5 shadow-[0_12px_24px_-18px_rgba(42,21,64,0.28)] ring-1 ring-white/80 sm:w-auto sm:shrink sm:rounded-[18px]"
        ...
        {/* Vision — what we almost built */}
        <section className="px-4 pb-6 sm:px-6">
          <div
            className="relative overflow-hidden rounded-[22px] p-5 shadow-[0_18px_38px_-22px_rgba(42,21,64,0.32)] ring-1 ring-white/80 backdrop-blur-md sm:rounded-[26px] sm:p-7"
            style={{
              background:
                "linear-gradient(160deg, #fff4f9 0%, #fff8ec 48%, #f3efff 100%)",
            }}
          >
            <div
              aria-hidden
              className="pointer-events-none absolute -top-16 -right-10 h-60 w-60 rounded-full opacity-60 blur-3xl"
              style={{ background: "radial-gradient(circle, #ffd1e8 0%, transparent 70%)" }}
            />
            <div
              aria-hidden
              className="pointer-events-none absolute -bottom-20 -left-14 h-64 w-64 rounded-full opacity-55 blur-3xl"
              style={{ background: "radial-gradient(circle, #e4d1ff 0%, transparent 70%)" }}
            />
            <span
              aria-hidden
              className="pointer-events-none absolute right-4 top-4 animate-[wiggle_2.4s_ease-in-out_infinite] text-[22px] text-[color:var(--accent)] sm:right-6 sm:top-6 sm:text-[28px]"
            >
              ✿
            </span>
            ...
            <div className="relative flex flex-col gap-3">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="inline-flex items-center gap-1 rounded-full bg-[#fff0d6] px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#92400e] ring-1 ring-[#fcd34d]/70">
                  {t.visionWarn}
                </span>
              </div>

              <h3 className="serif-italic text-balance text-[22px] font-semibold leading-[1.08] text-[color:var(--ink)] sm:text-[30px]">
                {t.visionTitle}
              </h3>

              <p className="serif-italic text-balance text-[14.5px] leading-[1.45] text-[color:var(--ink)] sm:text-[17px]">
                {t.visionLede}
              </p>

              <p className="text-balance text-[13px] leading-[1.6] text-[color:var(--ink-soft)] sm:text-[14.5px]">
                {t.visionBody}
              </p>

              <p className="serif-italic text-balance text-[13.5px] leading-[1.55] text-[color:var(--ink)] sm:text-[15.5px]">
                {t.visionClosing}
              </p>

              <div className="mt-2 flex flex-col gap-2 rounded-[16px] bg-white/75 p-3 ring-1 ring-white/80 sm:flex-row sm:items-start sm:gap-3 sm:p-4">
                <span className="inline-flex shrink-0 items-center gap-1.5 self-start rounded-full bg-[color:var(--accent)] px-3 py-1 text-[10.5px] font-semibold uppercase tracking-[0.2em] text-white shadow-[0_6px_14px_-6px_rgba(236,72,153,0.55)]">
                  {t.visionNextBadge}
                </span>
                <p className="text-balance text-[12.5px] leading-[1.55] text-[color:var(--ink-soft)] sm:text-[13.5px]">
                  {t.visionNext}
                </p>
              </div>

              <div className="mt-1 flex items-center gap-2 text-[color:var(--ink-muted)]">
        ...
        {/* Footer */}
        <footer className="order-[100] px-4 pb-[max(env(safe-area-inset-bottom),16px)] pt-1 sm:order-none sm:px-6">
          <div className="flex flex-col items-center justify-between gap-2 rounded-[18px] bg-white/70 px-4 py-2.5 shadow-[0_2px_10px_-4px_rgba(42,21,64,0.15)] ring-1 ring-white/80 backdrop-blur-md sm:flex-row sm:gap-2 sm:rounded-full sm:px-4 sm:py-2">
            <span className="serif-italic text-center text-[11.5px] text-[color:var(--ink-soft)]">
              {t.footerTagline}
            </span>
            <div className="flex items-center gap-1.5">
              <Link
                href="/gallery"
                className="btn-ghost px-3 py-1.5 text-[10.5px] font-medium sm:py-1"
              >
                {t.footerGallery}
              </Link>
              <Link
                href="/?onboarding=1"
                className="btn-primary px-3.5 py-1.5 text-[11px] font-semibold sm:py-1"
              >
                {t.footerBegin}
              </Link>
            </div>
